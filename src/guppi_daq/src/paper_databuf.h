...
const unsigned char missed_chan_mask[8] = {
	0x01, 0x02, 0x04, 0x08,
	0x10, 0x20, 0x40, 0x80
};
typedef struct {
    unsigned long long mcnt;
    unsigned char missed_chan[16];  // 128 bits : address byte as floor(chan/8)
				    // address bit w/in byte as chan%8 (see missed_chan_mask)
} paper_databuf_element_t;
#define NUM_DATABUF_ELEMENTS 3
typedef struct {
    long num_elements;
    paper_databuf_element_t element[NUM_DATABUF_ELEMENTS];
} paper_databuf_t;
...
{
    long long mcnt[3];  // mcnt for each of the 3 sub-blocks
};
